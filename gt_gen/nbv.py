...
# ---------------- v1 fallback：直线加权（§5，不算 P*/B） ----------------

def _point_segment_dist(pts, a, b) -> np.ndarray:
    """点集 pts(N,3) 到线段 a→b 的距离 (N,)。"""
    pts = np.asarray(pts, float); a = np.asarray(a, float); b = np.asarray(b, float)
...
def best_next_view_line_weighted(handle, cur_cfg, voxmap, truth_scene, goal_pose,
                                 params=None, camera_model=None, line_radius: float = 0.5) -> NBVResult:
    """v1 fallback（§5）：不算 P*/B。取"靠近 start→goal 直线的 UNKNOWN"当目标，朝它们生成候选，
    gain 按揭开的 UNKNOWN 体素离直线的反比加权 Σ 1/(1+d/line_radius)，argmax。

    便宜、鲁棒，但绕障不如 oracle-path 聪明。start = cur_cfg 的 EE 位置，goal = goal_pose 位置。
    """
    from gt_gen import curobo_iface as ci
    from gt_gen.candidates import generate_candidates
    from gt_gen.sensor import load_camera_model
    ...
    a = ci.fk(handle, cur_cfg)[0]                             # start = 当前 EE 位置
    b = np.asarray(goal_pose[0], float)                      # goal = 目标位置

    unk = voxmap.state_centers(UNKNOWN)
    if unk.shape[0] == 0:
        return NBVResult("corridor_confirmed")
    near = unk[_point_segment_dist(unk, a, b) < line_radius]
    if near.shape[0] == 0:
        return NBVResult("no_reachable_candidate")
    target_idx = voxmap.world_to_voxel(near)                 # 近直线的 UNKNOWN 当作 "B" 喂给候选生成

    cands = generate_candidates(handle, voxmap, target_idx, camera_model, cur_cfg)
    if not cands:
        return NBVResult("no_reachable_candidate", n_B=int(near.shape[0]))
    ...
    for c in cands:
        ...
        score = 0.0
        if reveal.shape[0]:
            is_unk = np.asarray(voxmap.get(reveal)) == UNKNOWN
            if is_unk.any():
                d = _point_segment_dist(voxmap.voxel_to_world(reveal[is_unk]), a, b)
                score = float(np.sum(1.0 / (1.0 + d / line_radius)))   # 越靠直线越值钱
        if score > best_score:
            best, best_score = c, score

    return NBVResult("ok", cfg=list(best.config), cam_pose=best.cam_pose, target=best.target,
                     gain=float(best_score), score=float(best_score), n_candidates=len(cands))
