...
def build_action_matrix_old(T_all, w_trans=1.0, w_rot=1.0):
    """
    ...
    Returns:
        M_action: (B, N, N) 矩阵，M_action[i, j] 表示从帧i到帧j的SE(3)距离
    """
    device = T_all.device
    ...
    # T_all_inv: (B, N, 4, 4)
    # T_all: (B, N, 4, 4)
    # 我们需要计算 T_all_inv[i] @ T_all[j] 对于所有 (i, j)
    # 使用广播：(B, N, 1, 4, 4) @ (B, 1, N, 4, 4) -> (N, N, 4, 4)
    T_all_inv_expanded = T_all_inv.unsqueeze(-3)  # (B, N, 1, 4, 4)
    T_all_expanded = T_all.unsqueeze(-4)  # (B, 1, N, 4, 4)
    T_rel_all = T_all_inv_expanded @ T_all_expanded  # (B, N, N, 4, 4)
    ...
    # 计算平移距离 (B, N, N)
    trans_distances = torch.norm(t_all, dim=-1)  # (B, N, N)
    
    # 计算旋转角度 (B, N, N)
    # 对于每个 (i, j)，计算 R_all[i, j] 的旋转角度
    trace = torch.diagonal(R_all, dim1=-2, dim2=-1).sum(dim=-1)  # (B, N, N)
    cos_theta = (trace - 1.0) / 2.0
    cos_theta = torch.clamp(cos_theta, min=-1.0, max=1.0)
    theta = torch.acos(cos_theta)  # (B, N, N)
    rot_distances = torch.abs(theta)  # (B, N, N)
    
    # 计算加权SE(3)距离
    M_action = w_trans * trans_distances + w_rot * rot_distances  # (B, N, N)
